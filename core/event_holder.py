...
class EventHolder :
    ...
    def get_events( self ) :
        ...
        for i in pg.event.get() :
            if i.type == pg.FINGERDOWN or i.type == pg.FINGERMOTION :
                self.fingers[i.finger_id] = [i.x * cr.screen.get_width(),
                    i.y * cr.screen.get_height()]

            if i.type == pg.FINGERUP :
                self.fingers[i.finger_id] = None
